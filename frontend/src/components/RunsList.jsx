...
function ConfidenceBar({ value }) {
  if (value === undefined || value === null) {
    return <span className="text-[10px] font-semibold text-slate-400">Confidence: n/a</span>;
  }
  ...
  return (
    <section className="card p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-slate-950">Run loglari</h2>
          <p className="text-sm text-slate-500">Platform, task eslesmesi ve aksiyon sonucu</p>
        </div>
          ...
          <div className="flex items-start gap-3 rounded-lg border border-line bg-white p-3" key={run.id}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                <MetaTag label="Platform:" value={run.platform} color="bg-blue-50 text-blue-700" />
                <MetaTag
                  label="Matched Task ID:"
                  value={run.matchedTaskId || run.taskId}
